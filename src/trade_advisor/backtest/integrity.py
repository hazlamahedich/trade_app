...
def check_integrity(
    ...
    *,
    ...
    if equity.empty or len(equity) < 2:
        ...
        if nan_mask.any():
            nan_idx = equity.index[nan_mask.values].tolist()  # type: ignore[index]
    ...
    neg_mask = equity < 0
    if neg_mask.any():
        neg_idx = equity.index[neg_mask.values].tolist()  # type: ignore[index]
    ...
    if len(returns) > 0:
        ...
        if spike_mask.any():
            spike_idx = returns.index[spike_mask.values].tolist()  # type: ignore[index]
    ...
    zero_mask = equity == 0
    if zero_mask.any():
        zero_idx = equity.index[zero_mask.values].tolist()  # type: ignore[index]
        warnings.append(
